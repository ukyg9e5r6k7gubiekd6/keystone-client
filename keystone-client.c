 ...
 */
static void
default_json_error_callback(const char *json_funcname,
							enum json_tokener_error json_err)
{
	...
	fprintf(stderr, "%s failed: libjson error %ld: %s\n", json_funcname,
			(long) json_err, json_tokener_error_desc(json_err));
}
...
json_array_length(keystone_context_t *context, struct json_object *array)
{
	...
	if (json_object_is_type(array, json_type_array)) {
		...
		if (len < 0) {
			context->keystone_error("JSON array length is negative",
									KSERR_PARSE);
			len = 0;
		} else if ((unsigned int)len > UINT_MAX) {
			context->keystone_error(
					"JSON array length is too large for unsigned int",
					KSERR_PARSE);
			len = 0;
		}
	} else {
...
json_array_get(keystone_context_t *context, struct json_object *array,
		unsigned int index)
{
	...
	if (json_object_is_type(array, json_type_array)) {
		if (index < json_array_length(context, array)) {
			...
			if (NULL == item) {
				context->keystone_error("failed to index into JSON array",
										KSERR_PARSE);
			}
		} else {
			context->keystone_error("JSON array index out of bound",
									KSERR_PARSE);
			item = NULL;
		}
	} else {
 ...
 */
typedef item_callback_return_t (*item_callback_func_t)(
		keystone_context_t *context, struct json_object *item,
				void *callback_arg);

/**
 * Iterate over the given JSON array, passing elements of it to the given
 * iteration function. If the iteration function ever returns STOP, return the
 * array element passed to it which caused it to first return STOP. If the
 * iteration function returns CONTINUE for each and every array element, return
 * NULL. The iteration function is not guaranteed to be called for each and
 * every element in the given array.
 */
static struct json_object *
json_array_find(keystone_context_t *context, struct json_object *array,
		item_callback_func_t callback, void *callback_arg)
{
	struct json_object *item;
	unsigned int i;
...
/**
 * If the OpenStack service represented by the given JSON object is of the type
 * name given by callback_arg, return STOP. Otherwise, if it is of some other
 * type or if its type cannot be determined, return CONTINUE.
 */
static item_callback_return_t
filter_service_by_type(keystone_context_t *context, struct json_object *service,
		void *callback_arg)
{
	...
	if (!json_object_is_type(service, json_type_object)) {
		context->keystone_error(
				"response.access.serviceCatalog[n] is not an object",
				KSERR_PARSE);
		return CONTINUE;
	}
	if (!json_object_object_get_ex(service, "type", &service_type)) {
		context->keystone_error(
				"response.access.serviceCatalog[n] lacks a 'type' key",
				KSERR_PARSE);
		return CONTINUE;
	}
	if (!json_object_is_type(service_type, json_type_string)) {
		context->keystone_error(
				"response.access.serviceCatalog[n].type is not a string",
				KSERR_PARSE);
		return CONTINUE;
	}
...
/**
 * Given a JSON array representing a list of OpenStack services, find the first
 * service of the given-named type. If a service of the given type name is
 * found, return it. Otherwise, if no service of the given type name is found,
 * return NULL.
 */
struct json_object *
find_service_by_type_name(keystone_context_t *context,
								  ...
								  const char *desired_type)
{
	...
	return json_array_find(context, services, filter_service_by_type,
						   (void *) desired_type);
}

/**
 * Given a JSON array representing a list of OpenStack services, find the first
 * service of the given type. Otherwise, if a service of the given type is
 * found, return it. Otherwise, if no service of the given type is found, return
 * NULL.
 */
struct json_object *
find_service_by_type(keystone_context_t *context, struct json_object *services,
		enum openstack_service desired_type)
{
	assert(context != NULL);
	assert(services != NULL);
	...
	return json_array_find(
			context, services, filter_service_by_type,
			(void *) openstack_service_names[(unsigned int) desired_type]);
}

/**
 * Given a JSON object representing an endpoint of an OpenStack service and a
 * desired endpoint version, if the given endpoint appears to have the given API
 * version, or it has no versionID attribute, return STOP. Otherwise, if the
 * endpoint's version is not the desired version, or the endpoint's version is
 * erroneous, return CONTINUE.
 */
static item_callback_return_t
filter_endpoint_by_version(keystone_context_t *context, json_object *endpoint,
						   void *callback_arg)
{
	...
	if (!json_object_is_type(endpoint, json_type_object)) {
		context->keystone_error(
				"response.access.serviceCatalog[n].endpoints[n] is not an "
				"object",
				KSERR_PARSE);
		return CONTINUE;
	}
	if (!json_object_object_get_ex(endpoint, "versionId",
								   &endpoint_api_version)) {
		/* Keystone documentation includes a versionID key, but it is not
		 * present in the responses I've seen context->keystone_error(
		 * "response.access.serviceCatalog[n].endpoints[n] lacks a 'versionId'
		 * key", KSERR_PARSE); */
		return STOP; /* Take a lack of versionID to mean a catch-all */
	}
	if (!json_object_is_type(endpoint_api_version, json_type_string)) {
		context->keystone_error(
				"response.access.serviceCatalog[n].endpoints[n].versionId is "
				"not a string",
				KSERR_PARSE);
...
/**
 * Given a JSON object representing an OpenStack service, find the first
 * endpoint of the given version. If an endpoint of the given version is found,
 * return it. Otherwise, if no endpoint of the given version is found, return
 * NULL.
 */
static struct json_object *
service_find_endpoint_by_version(keystone_context_t *context,
										 ...
										 unsigned int desired_api_version)
{
	...
	if (json_object_object_get_ex(service, "endpoints", &endpoints)) {
		if (0 == desired_api_version) {
			/* No desired API version currently set, so use the first endpoint
			 * found */
			endpoint = json_array_get(context, endpoints, 0);
		} else {
			/* Looking for a certain version of the Swift RESTful API */
			endpoint = json_array_find(context, endpoints,
									   filter_endpoint_by_version,
									   (void *) &desired_api_version);
		}
	} else {
		context->keystone_error("response.access.serviceCatalog[n] lacks an "
								"'endpoints' key",
								KSERR_PARSE);
...
/**
 * Given a JSON object representing an OpenStack service endpoint, return its
 * URL of the given type, if any.
 * If the service endpoint has no URL
 */
static const char *
endpoint_url(keystone_context_t *context, struct json_object *endpoint,
		enum openstack_service_endpoint_url_type endpoint_url_type)
{
	...
	assert(context != NULL);
	assert(endpoint != NULL);
	assert(endpoint_url_type < ELEMENTSOF(
			openstack_service_endpoint_url_type_names));
	assert(openstack_service_endpoint_url_type_names
	[(unsigned int)endpoint_url_type] != NULL);

	url_type_name = openstack_service_endpoint_url_type_names
			[(unsigned int) endpoint_url_type];

	if (json_object_object_get_ex(endpoint, url_type_name,
								  &endpoint_public_url)) {
		if (json_object_is_type(endpoint_public_url, json_type_string)) {
			url_val = json_object_get_string(endpoint_public_url);
		} else {
			context->keystone_error(
					"response.access.serviceCatalog[n].endpoints[n] URL is not "
					"a string",
					KSERR_PARSE);
			url_val = NULL;
		}
		...
		context->keystone_error(
				"response.access.serviceCatalog[n].endpoints[n] lacks a URL "
				"key of the requested type",
				KSERR_PARSE);
		url_val = NULL;
	}
...
/**
 * Given a desired service type and version and type of URL, find a service of
 * the given type in Keystone's catalog of services, then find an endpoint of
 * that service with the given API version, then return its URL of the given
 * type. Return NULL if the service cannot be found, or if no endpoint of the
 * given version can be found, or if the service endpoint of the given version
 * has no URL of the given type.
 */
const char *
keystone_get_service_url(
				...
				enum openstack_service_endpoint_url_type endpoint_url_type)
{
	...
	service = find_service_by_type(context, context->pvt.services,
								   desired_service_type);
	if (service) {
		static struct json_object *endpoint;
		endpoint = service_find_endpoint_by_version(context, service,
													desired_api_version);
		if (endpoint) {
...
/**
 * Retrieve the authentication token and service catalog from a now-complete
 * Keystone JSON response, and store them in the Keystone context structure for
 * later use.
 */
...
process_keystone_json(keystone_context_t *context, struct json_object *response)
{
	...
	if (context->pvt.debug) {
		json_object_to_file_ext("/dev/stderr", response,
	...
	if (!json_object_object_get_ex(token, "catalog", &context->pvt.services)) {
		context->keystone_error("response.token lacks 'catalog' key",
								KSERR_PARSE);
		return KSERR_PARSE;
	}
	if (!json_object_is_type(context->pvt.services, json_type_array)) {
		context->keystone_error("response.token.catalog not an array",
								KSERR_PARSE);
		return KSERR_PARSE;
	}
 ...
 * This parses the response and saves copies of the interesting service endpoint
 * URLs.
 */
static size_t
process_keystone_response(void *ptr, size_t size, size_t nmemb, void *userdata)
{
	...
	if (json_tokener_success == json_err) {
		...
		if (sc_err != KSERR_SUCCESS) {
			return 0; /* Failed to process JSON. Inform libcurl no data
 * 'handled' */
		}
		...
		return 0; /* Apparent JSON parsing problem. Inform libcurl no data
 * 'handled' */
	}
 ...
 * and user names and password. This yields an authorisation token, which is
 * then used to access all Swift services.
 */
enum keystone_error
					  ...
					  const char *password)
{
	...
	if (NULL == context->pvt.json_tokeniser) {
		...
		if (NULL == context->pvt.json_tokeniser) {
			context->keystone_error("json_tokener_new failed",
									KSERR_INIT_FAILED);
			return KSERR_INIT_FAILED;
		}
	} else {
	...
	curl_err = curl_easy_setopt(context->pvt.curl, CURLOPT_WRITEFUNCTION,
